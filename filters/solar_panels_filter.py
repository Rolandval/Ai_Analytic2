...
- "brand": бренд
- "power": потужність (Вт | None)
- "full_name": повна назва
- "price": ціна в доларах США
- "panel_type": тип панелі (одностороння/двостороння | None)
- "cell_type": тип комірок (n-type/p-type | None)
- "thickness": товщина (мм | None)
...
- якщо ти бачиш що потужність є занадто великою (більше 2000 Вт) або занадто малою (менше 10 Вт) то пропусти цю панель
- якщо ти бачиш що товщина є занадто великою (більше 500 мм) або занадто малою (менше 10 мм) то пропусти цю панель
Проаналізуй поле `full_name`, і якщо:
- можеш визначити потужність з назви, якщо вона відсутня або некоректна
- інші параметри можеш також проаналізувати та виправити 
- якщо поле `panel_type` є None то постарайся знайти це поле в ful_name якщо це можливо, якщо ні то None
- якщо поле `cell_type` є None то постарайся знайти це поле в ful_name якщо це можливо, якщо ні то None
- якщо поле `panel_color` є None то постарайся знайти це поле в ful_name якщо це можливо, якщо ні то None
- якщо поле `frame_color` є None то постарайся знайти це поле в ful_name якщо це можливо, якщо ні то None
...
- "panel_color": panel_color(str)
- "frame_color": frame_color(str)
- "price_per_w": price_per_w(float)
