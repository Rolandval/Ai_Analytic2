...
from db.models import BatteriesBrands
...
async def get_or_create_brand(session: AsyncSession = AsyncSession(), brand_name: str = "") -> int:    
    ...
    query = select(BatteriesBrands).where(BatteriesBrands.name == brand_name_upper)
    ...
    new_brand = BatteriesBrands(name=brand_name_upper)
