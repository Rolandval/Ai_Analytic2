...
def get_page_url(page: int) -> str:
    return f"https://shyp-shyna.com.ua/catalogs/batteries/filter/brand-a_mega+exide+topla+westa/?page={page}"
...
def extract_batteries_links_from_html(html: str) -> List[str]:
    ...
    container = soup.find("div", class_="catalog-list js-catalog-main")
    if not container:
        print("⚠️ Контейнер не знайдено")
        return []
    batteries = container.find_all("div", class_="goods-tile")
    batteries_links = []
    for battery in batteries:
        link_div = battery.find("a", class_="goods-tile__name")
    ...
    try:
        async with session.get(url) as response:
            ...
            battery_details = soup.find("div", {
                "class": "product-section__body",
            })
            price_div = soup.find("div", {
                "class": "product-price__cost",
            })
...
async def parse_batteries_shyp_shuna() -> List[str]:
    headers = get_headers()
...
#     result = asyncio.run(parse_batteries_shyp_shuna())
#     print(result)