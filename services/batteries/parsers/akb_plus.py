...
def get_page_url(page: int) -> str:
    return f"https://akb-plus.com/ua/akb/brandans/bosch/electric-power/tab?page={page}"
...
async def get_last_page() -> int:
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        url = "https://akb-plus.com/ua/akb/brandans/bosch/electric-power/tab"
        html, _ = await fetch_html(session, url, 0)
        soup = BeautifulSoup(html, 'html.parser')
        pagination_div = soup.find("div", class_="results pull-left")

        if pagination_div:
            text = pagination_div.text.strip()
            # Шукаємо число в дужках (кількість сторінок)
            import re
            match = re.search(r'\((\d+)\s+сторінок\)', text)
            if match:
                return int(match.group(1))
        
        # За замовчуванням повертаємо 1 сторінку, якщо не вдалося знайти
        return 1
...
def extract_batteries_links_from_html(html: str) -> List[str]:
    ...
    batteries = soup.find_all("div", class_="product-thumb")
    batteries_links = []
    for battery in batteries:
        batteries_links.append(battery.find("a")["href"])
...
async def parse_batteries_akb_plus() -> List[str]:
    headers = get_headers()
...
#     last_page = asyncio.run(parse_batteries_akb_plus())
#     print(last_page)
